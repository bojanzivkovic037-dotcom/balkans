...
app.post('/api/registracija', async (req, res) => {
  ...
  const { data, error } = await supabase.auth.admin.createUser({ email, password: lozinka, email_confirm: true });
  if (error) return res.status(400).json({ greska: error.message });
  await supabase.from('profiles').insert({ id: data.user.id, email, ime, prezime, odakle: '', destinacija_grad: '', destinacija_drzava: '' });
  res.json({ uspeh: true, korisnik: data.user });
});
...
app.get('/api/poruke/:korisnik_id', async (req, res) => {
  ...
  const { data, error } = await supabase.from('poruke').select('*').or(`posiljalac_id.eq.${id},primalac_id.eq.${id}`).order('created_at', { ascending: true });
  if (error) return res.json([]);
  res.json(data || []);
});
